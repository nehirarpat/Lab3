...
outliers/patterns, or sample size. We can see that both RSE plots are
left tailed with the male plot having two modalities. For SWL plot (also
left tailed), we can see that the distribution for males and females are
pretty similar, so we cannot tell whether difference between the means
are significant or not without a statistical test.
